...
START_OUTBOX="RUST_LOG=info cargo watch --env-file ./.env -- cargo run --bin outbox"
...
if ! tmux has-session -t $SESH 2>/dev/null; then
  ...
  tmux new-window -t $SESH -n "outbox"
  tmux send-keys -t $SESH:outbox "cd $BACKEND_PATH" C-m
  tmux send-keys -t $SESH:outbox "$START_OUTBOX" C-m
